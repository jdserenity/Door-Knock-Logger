...
function LogItem({ log, previousTimestamp, onDelete }) {
  ...
  return (
    <animated.div
      ...
      className="relative py-3 border-t border-gray-800 bg-gray-900 cursor-grab overflow-hidden"
    >
      <animated.div 
      ...
      <animated.div className="relative z-10 bg-gray-900 px-2">
        {/* Use 3-column Grid for layout */}
        <div className="grid grid-cols-3 items-center text-lg gap-x-2">
          <span className="text-left truncate italic">{log.doorNumber}, {log.streetName}</span>
          <span className="text-center">{formattedStatus}</span>
          {/* Apply italic only if previousTimestamp exists (i.e., it's elapsed time) */}
          <span 
            className={`text-right text-gray-400 text-sm ${previousTimestamp ? 'italic' : ''}`}
          >
            {timeDisplay}
          </span>
        </div>
